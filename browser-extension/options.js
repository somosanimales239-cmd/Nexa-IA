...
async function load() {
  const settings = await chrome.storage.local.get({ apiBase:'http://127.0.0.1:32145', token:'' });
...
document.getElementById('save').addEventListener('click', async () => {
  try {
    ...
    await chrome.storage.local.set({ apiBase:base, token:value });
    ...
    show(`Conectado a Nexa AI ${data.appVersion || ''} / API ${data.apiVersion || '1'}.`, 'ok');
