...
const ERROR_REPAIRS = Object.freeze({
  ...
  E002: {
    positive: ['exact requested subject count', 'one clear composition', 'no additional subjects'],
    negative: ['wrong number of subjects', 'extra subject', 'character sheet', 'turnaround sheet', 'lineup', 'multiple views', 'multiple poses of the same subject'],
  },
  E003: {
    positive: ['exactly one requested subject when one subject was requested', 'single isolated subject', 'one pose only', 'no duplicated character'],
    negative: ['duplicate subject', 'cloned subject', 'second character', 'multiple copies', 'character sheet', 'turnaround sheet', 'lineup', 'multiple views', 'multiple poses'],
  },
  E004: {
  ...
  E008: {
    positive: ['unmistakable requested species identity', 'species-defining anatomy and silhouette must be clearly visible'],
...
function buildEvaluationPrompt({ userRequest, plan, attempt = 1 }) {
  return [
    ...
    'repair_positive and repair_negative must contain short image-prompt phrases that directly address visible failures.',
    'Never invent a new requirement that is absent from the original request. Anthropomorphic animals are allowed unless the user explicitly forbids them.',
    'If E008 wrong species is present, repair_positive MUST include 3 to 6 concrete anatomy/silhouette cues that make the requested species unmistakable.',
    'If E002 or E003 is present because the image looks like a character sheet, lineup, turnaround, or repeated poses, explicitly place those concepts in repair_negative.',
    'Return no prose outside the required JSON.',
    '',
...
function applyRepairs(plan, evaluation, attempt = 2) {
  ...
  // Species-aware hard repair for the first validation target. This is deliberately
  // narrow and only activates when the prompt itself requests a kangaroo/canguro.
  const promptText = `${plan?.positive_prompt || plan?.positivePrompt || ''}`.toLowerCase();
  if (result.error_codes.includes('E008') && /kangaroo|canguro/.test(promptText)) {
    positive.push(
      'unmistakable kangaroo anatomy',
      'long upright kangaroo ears',
      'elongated kangaroo muzzle',
      'very large muscular balancing tail clearly visible',
      'powerful oversized hind legs and feet',
      'shorter forearms relative to hind legs'
    );
    negative.push(
      'bird beak', 'eagle head', 'human face', 'pig face', 'boar face',
      'bear muzzle', 'short ears', 'missing tail', 'short tail', 'wrong animal'
    );
  }
  if (result.error_codes.includes('E002') || result.error_codes.includes('E003')) {
    positive.push('one subject only', 'one pose only', 'single centered composition');
    negative.push('character sheet', 'turnaround sheet', 'reference sheet', 'lineup', 'multiple views', 'multiple poses');
  }
